/**
 * Assessment Routes for ThriveOrtho
 * Separate route handlers for assessment workflows
 */

import { Hono } from 'hono'

const assessmentApp = new Hono()

// Initial Assessment Route
assessmentApp.get('/initial-assessment', (c) => {
  return c.html(`
    <!DOCTYPE html>
    <html lang="en">
    <head>
                    ...
                    contentDiv.innerHTML = '<div class="space-y-4">' +
                        ...
                        '<div class="grid grid-cols-2 gap-4">' +
                            ...
                            '<div class="bg-purple-50 rounded-lg p-3">' +
                                ...
                                '<div class="text-xl font-bold text-purple-800">' + (results.redFlags ? results.redFlags.length : 0) + '</div>' +
                            '</div>' +
                        '</div>';
                    
                    if (results.redFlags && results.redFlags.length > 0) {
                        contentDiv.innerHTML += '<div class="bg-red-50 rounded-lg p-4">' +
                            '<h4 class="font-bold text-red-800 mb-2">Red Flags Detected</h4>';
                        
                        results.redFlags.forEach(function(flag) {
                            contentDiv.innerHTML += '<div class="text-red-700 text-sm mb-1">• ' + flag.description + '</div>';
                        });
                        
                        contentDiv.innerHTML += '</div>';
                    }
                    
                    contentDiv.innerHTML += '<div class="bg-gray-50 rounded-lg p-4">' +
                        '<h4 class="font-bold text-gray-800 mb-2">Next Steps</h4>';
                    
                    if (results.nextSteps) {
                        results.nextSteps.forEach(function(step) {
                            contentDiv.innerHTML += '<div class="flex justify-between items-center py-2 border-b border-gray-200 last:border-b-0">' +
                                '<span class="text-gray-700">' + step.action + '</span>' +
                                '<span class="text-sm text-gray-600">' + step.timeframe + '</span>' +
                                '</div>';
                        });
                    } else {
                        contentDiv.innerHTML += '<p class="text-gray-600">Follow-up as clinically indicated</p>';
                    }
